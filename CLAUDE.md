...
There is no CI. The only automated tests cover the host-side lot store (see below):
```bash
...
- `lotstore.py` holds the storage, validation and merge logic and has no UI code. `run_miqscore.py` is the
  ...
  subsampling, one `docker run` per `*_R1/_R2.fastq.gz` pair, and writes `miqscore_summary.csv` and
  `run_info.json` into the dated run folder.
